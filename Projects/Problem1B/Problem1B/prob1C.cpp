...
void bigint192_karatsuba_mul(bigint384 r, const bigint192 a, const bigint192 b)//complete this function
{
	...
	printf("cycles = %0.f cycles\n", (double)(cycles) * 2500000000 / (CLOCKS_PER_SEC * 20000));
}
...
int main()
{
	...
	//We are using a non-prime
	//P=(2^K) = (2^16)
	//Our k = 16
	/*The operation below represents the first 16 LSB's. The loop adds the values at each bit position (e.g. (2^0+2^1+....2^16)
	The result is the total remaining and would give the same result if operand modulus % was used*/
/*
	int mod = 0;
